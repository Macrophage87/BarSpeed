...
- **A green suite is not a run.** `UP-TO-DATE` and `FROM-CACHE` mean nothing executed; `--rerun-tasks` when a number matters. `:app` and `:core:ble` have no test source sets and there is no `androidTest` directory, so `./gradlew test` compiles them and asserts nothing. `:core:data` is different — it has had a test source set since `d69f299`/`52ccb55` (36 tests, 72 executions across both build variants) — but do not overcorrect: its tests take a fake DAO and never touch Room, SQLite or Android, so a green run there still proves nothing about platform behaviour, only about `SessionRepository`'s own mapping. CI's real command is unrestricted `./gradlew test` (`ci.yml:31`, 278 tests measured at `302e64fe0dcfb2cd0ea41774badeef930cd33c5d`), not `-PjvmOnly test` (206 at the same SHA) — the faster flag silently drops `:core:data`'s 72 executions along with the two untested modules.
