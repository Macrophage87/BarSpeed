...
Interpret these strictly. Work here is tracked by GitHub issues — address by number when one exists (`gh issue list --repo Macrophage87/BarSpeed --state all --limit 300 --json number,title,state` is the live list, field-selected per `.claude/facts/live-state.md` §16; do not trust a count written in this file, it is a claim about the past) and by name or by the task the owner stated otherwise. Read a single issue with `gh issue view N --json title,body`, adding `,comments` when the thread matters — see `.claude/facts/live-state.md` §16 for why. An issue body pins its `file:line` claims to the SHA it was audited against, so re-verify them against current `origin/main` before acting. Do not assume an issue is right: #22's prescribed two-line fix did not fix the defect it described.
