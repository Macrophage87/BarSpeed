...
Escalate on your own initiative, without waiting to be asked, when: the issue's or the design's own diagnosis turns out to be wrong (this has fired for real — #22's prescribed two-line fix did not fix the defect it described; #21 is the more nuanced instance, worth knowing exactly because it is not the clean case — its title misdiagnosed the throw as `SecurityException` alone, but its own Fix section correctly named `ForegroundServiceStartNotAllowedException` too, so the fix's diagnosis was actually right where the title was wrong, and what it truly missed was a second call site the fix never mentions); **two consecutive rounds find defects in the fix rather than the original defect**; a round produces a false claim about platform behaviour, which this repo has shipped three times; or scope grows beyond the sites named when the work started.
...
**Never state the denominator bare — name the parent SHA every time.** Measured at `eb88bf31c6f2705e1a24776e789d0d7fa010f099` it is 136 — core/model 86, core/dsp 34, core/hrm 10, core/witmotion 6 — but that figure is already this definition's second correction: it read 71 when first written (true at `e697787`), and the true count moved 71 → 80 → 84 → 94 → 97 → 106 → 111 → 127 → 132 → 136 across the next dozen-odd commits (as recorded in each commit's own body) while the stale 71 kept circulating in these agent definitions. Re-measure with `-PjvmOnly test --rerun-tasks` before trusting any number written here, including this one. This matters more here than in the Connect IQ repo: there is no baseline, no approval file, no coverage floor, and **no test-name pin file** — `scripts/` contains only `protect-branch.ps1` and `protect-branch.sh`. Do not invent one and do not pretend one exists. The substitute is manual and must be stated as manual: record the test total before and after every change, with its SHA, and name any test you added, renamed or removed in the commit body. Widening `reps.size in 4..6` to `3..7` is invisible to CI.
...
./gradlew -PjvmOnly test                    # 4 modules — re-measure and name the SHA; do not trust a cached number
