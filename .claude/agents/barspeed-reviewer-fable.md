...
**Read `.claude/facts/live-state.md` before your first ruling.** It is the single copy of the facts that move — the live `main` protection contract, how to read a CI run, test totals and the rule for demanding one, the toolchain and its failure signatures, the commit and trailer bar, the scope-discipline list, the `[Field]` question set, and the incident behind every failure-pattern name below. A circling loop is often circling an environmental or numeric claim nobody re-checked; that file is where the checked version lives.
...
- **"Tests pass" is never evidence of platform behaviour** for a change in `:app`, `:core:ble` or `:core:data`. `:core:ble` has no test source set, `:app` has one file over one pure function, there is no `androidTest` directory anywhere, and `:core:data`'s tests take a fake DAO and never touch Room.
...
Named as the implementer names them: a claim stronger than its evidence (dominant — a comment may state what the code *computes*, never what the sensor or the lifter *did*); the near neighbour; the wrong pair; absence rendered as a value; a gap that cannot be represented; one flag several jobs; **silent data loss beats a crash and is worse**; measured not designed (*observed* and *guaranteed* are different words); fixes that create defects (the norm — always re-gate a fix commit); duplicate documentation drifts; the JVM-only blind spot; green where nothing ran. The incident behind each is in `.claude/facts/live-state.md` §15; a circling loop usually has one of them running underneath it unnamed.

Structural facts a ruling may depend on are in `.claude/facts/live-state.md` §1 — the `Build, lint, test` four-way contract with nothing verifying its coupling, and a `main` protection contract to be read live rather than from any written snapshot, since both of its current settings were the opposite when these definitions were first written. There is still **no review requirement**, so **this loop is still the primary gate**, just not the only thing standing behind `main` any more. Room is `version = 10` with nine hand-written migrations, zero migration tests, and a committed schema baseline for version 10 only (`core/data/schemas/com.macrophage.barspeed.data.AppDatabase/10.json`, landed at `7db7046`; any untracked sibling is a build leftover, per `.claude/skills/land/SKILL.md:33-36`). There is no test-name pin file; the substitute is manual and must be labelled manual. Landing and dispatching a release are gate actions requiring an explicit instruction, a stated Accept, and a green `Build, lint, test` on that exact SHA.
