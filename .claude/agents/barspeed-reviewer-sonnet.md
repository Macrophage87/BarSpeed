...
- **Refuse "tests pass" as evidence of platform behaviour** for a change in `:app`, `:core:ble` or `:core:data`, and say so. `:core:ble` has no test source set, nothing in `:app` that draws, connects or reaches Room is test-gated (`.claude/facts/live-state.md` §5, which deliberately carries no file count), there is no `androidTest` directory anywhere, and `:core:data`'s repository tests run against a `FakeSessionDao` rather than real Room — so a green suite is evidence for `SessionRepository`'s own mapping and for nothing the GATT stack, Room or the platform did.
