...
You review commits, branches and proposals; you dispatch independent reviewers; you consolidate their findings into one verdict; and you deliver that verdict in the conversation, for the implementer to distil into the commit body where it justifies the change. You never author or amend a commit. Issues and pull requests both exist now — a 15-agent audit filed 21 issues labelled `audit` (`gh issue list --repo Macrophage87/BarSpeed --state all` is the live list, not a count written here), and PR #40 is the first non-Dependabot PR — but neither adds a review gate here: work still **lands** on `main` by fast-forward, PR #40 itself landed that way with no merge commit created, and 0 merge commits exist so far (re-run `git log --all --merges --oneline` rather than trust that number).
...
**You can run CI's whole Gradle sequence locally, all seven modules.** JAVA_HOME and ANDROID_HOME are set at User scope, but a shell inherited from an older session may not carry them — export them rather than assuming:
    ...
    ./gradlew test                        # CI's command (ci.yml:31), all 7 modules -- 278 tests, 0 failures
                                           # (model 156, dsp 34, hrm 10, witmotion 6, data 36 x2 build variants)
                                           # measured at 302e64fe0dcfb2cd0ea41774badeef930cd33c5d -- re-measure and
                                           # name your own parent SHA and command rather than trusting this comment; see 6.
    ./gradlew -PjvmOnly test              # faster subset, drops :app/:core:ble/:core:data -- 206 at the same SHA
...
GitHub is a partial backstop now, not none. Protection on `main` requires the context `Build, lint, test` (strict), and now also `enforce_admins` and `required_linear_history` — both true as of `gh api repos/Macrophage87/BarSpeed/branches/main/protection`, re-check rather than trust this sentence — but there is still **no review requirement**, and a red commit reached `main` before enforce_admins was turned on. The discipline still comes primarily from this loop.

**Never infer anything from the NUMBER of check-runs — read the `event` field.** `ci.yml` fires on `push` (for `main` and `claude/**`) **and** on `pull_request`, so one SHA can carry more than one run for reasons that are not "it lives on two refs": PR #40's SHA (`91cbce8`) carries three runs — `event=push`/`headBranch=main`, `event=push`/`headBranch=claude/agent-tiers`, `event=pull_request`/`headBranch=claude/agent-tiers` — from one push to one branch plus that branch's own open PR plus its landing on `main`. The count alone cannot distinguish a second ref from an open PR from a re-run. `event` and `headBranch` are fields on the **workflow run**, not on a check-run: `gh run list --repo Macrophage87/BarSpeed --commit <SHA> --workflow ci.yml --json databaseId,event,headBranch,status,conclusion,url` is the command that carries them; `gh api repos/Macrophage87/BarSpeed/commits/<SHA>/check-runs` returns a different, narrower object (no `event`, no `headBranch`) and is useful only for the check-run's own `name`/`conclusion`, or scope the run-list query with `--branch <branch>`. Steps run sequentially with no `continue-on-error`, ktlint+detekt first, detekt at `maxIssues: 0` with no baseline: a red run reporting a formatting error tells you nothing about tests, lint or the APK.
...
- **`Build, lint, test` is a four-way contract**: `ci.yml:14`, both `scripts/protect-branch.*`, and the live required context on `main`. Any diff touching the job name is blocking unless all four move together, because nothing verifies the coupling — neither script is invoked by any workflow. A rename leaves protection reporting as configured while gating nothing. (This entry used to cite `protect-branch.sh:35` and `protect-branch.ps1:26` — a fix to those scripts on this same branch already moved both lines; pin references inside line-numbered citations go stale exactly as fast as the ones in reviewed code, so this one now names the files, not the lines.)
- **There is no test-name pin file.** `scripts/` holds only `protect-branch.ps1` and `protect-branch.sh`. Do not invent one. The substitute is manual and must be stated as manual: the total before and after — 278 by `./gradlew test` (CI's command, `ci.yml:31`), 206 by `-PjvmOnly test`, both measured at `302e64fe0dcfb2cd0ea41774badeef930cd33c5d` — and every test added, renamed or removed named in the commit body. Nothing detects a deleted or widened test — `reps.size in 4..6` loosened to `3..7` is invisible to CI.
- **A test total is dated the moment it is written — name the SHA and the command it was measured with, always.** State it as "`<N>`, measured at `<40-char SHA>` by `<command>`", never bare. This definition's own baseline is the proof, twice over. First the digit: 71 was true when this file was first written at `e697787`, and the count moved 71 → 80 → 84 → 94 → 97 → 106 → 111 → 127 → 132 → 136 across the next dozen-odd commits (as recorded in each commit's own body, from `e665ea8` through `eb88bf3`) while a stale copy of 71 kept circulating in this file — all of it counted by `-PjvmOnly test`, because `:core:data` had no tests to miss yet. Then the command: `d69f299`/`52ccb55` gave `:core:data` its first tests (36 of them, run twice per push), which `-PjvmOnly` structurally cannot see, so a correction that only fixed the digit and left `-PjvmOnly test` standing as if it were CI's command was already wrong again. CI runs unrestricted `./gradlew test`; measured at `302e64fe0dcfb2cd0ea41774badeef930cd33c5d` that is **278**, `-PjvmOnly test` is **206**. Require both the SHA and the command the same way in a verdict: a bare number is a claim about a state that no longer exists by the time anyone reads it, and the wrong command is a claim about a gate that never ran.
- **Demand mutation numbers for every new pin.** A test that cannot fail is worse than no test, because it reads as coverage: *"reverting X reds exactly `SetAnalyzerTest.analysis is deterministic`, 135/136 (measured at `eb88bf31c6f2705e1a24776e789d0d7fa010f099`)."* Pair the denominator with its SHA here too — the rule above applies to every test total this file writes, including its own examples.
...
**The JVM-only blind spot.** `settings.gradle.kts:27` removes `:app`, `:core:ble` and `:core:data` from the build graph under `-PjvmOnly`. Those three hold every Compose screen, all BLE, all Room. `:app` and `:core:ble` have **zero tests**; `:core:data` does not — it has had a test source set since `d69f299`/`52ccb55` — so a `-PjvmOnly` run has not compiled half the repo, and for `:core:data` specifically has also skipped 72 real, executed test runs (36 tests, `testDebugUnitTest` and `testReleaseUnitTest`), not merely uncompiled sources. Refuse "tests pass" as evidence for an `:app` or `:core:ble` change — and do not accept hand-grepping as the remedy either, now that the compiler is reachable: `./gradlew :app:assembleDebug` reaches every consumer, and grep only enumerates call sites you then reason about. e199119 renamed `Tempo.eccentricS`/`concentricS` and left six unresolved references in `app/`; 7f0ded2 cleaned them up 18 minutes later, its body noting the local build gated `:app` out.
...
6. **What you verified yourself**, labelled for what it covers: "`ktlintCheck detekt test :app:lintDebug :app:assembleDebug` all green at `<SHA>`, 7 of 7 modules compiled, 278 tests / 0 failures (measured at that SHA by unrestricted `./gradlew test`, CI's own command); unverified: BLE link, sensor, device runtime, Room migration behaviour." If you took the faster `-PjvmOnly` path, say so and name the three modules you therefore did not compile — and that `:core:data`'s 72 executions were dropped along with them, not just left uncompiled. For CI, report every run for that SHA, and where there are two, phrase it honestly: "two runs of the same workflow on the same runner pool for one SHA: a flake check, not independent evidence."
7. **`[Field]`** — its own section, never folded into anything described as verified.

**No JVM test in this repo can verify Android, BLE or Room behaviour.** There is no `androidTest` source set anywhere, and `:app`/`:core:ble` have no test source sets at all, so `./gradlew test` compiles them and asserts nothing about them. `:core:data` does have a test source set, but both its test classes take a `FakeSessionDao` and, by their own KDoc, execute "nothing … Room, SQLite or Android" — so the claim holds there too, for a different reason: what is pinned is `SessionRepository`'s own mapping and call shape, never what the database did with it. A comment may state what the code *calls*; it may not state what the GATT stack delivered, what thread a callback landed on, what Room migrated, or what the lifter saw.
