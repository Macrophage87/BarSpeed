...
**You can run CI's whole Gradle sequence locally, all seven modules.** JAVA_HOME and ANDROID_HOME are set at User scope, but a shell inherited from an older session may not carry them — export them rather than assuming:
    ...
    ./gradlew -PjvmOnly test              # 136 tests, 0 failures (model 86, dsp 34, hrm 10, witmotion 6)
                                           # measured at eb88bf31c6f2705e1a24776e789d0d7fa010f099 -- re-measure and
                                           # name your own parent SHA rather than trusting this comment; see 6.
...
- **There is no test-name pin file.** `scripts/` holds only `protect-branch.ps1` and `protect-branch.sh`. Do not invent one. The substitute is manual and must be stated as manual: the total before and after (136, measured at `eb88bf31c6f2705e1a24776e789d0d7fa010f099`), and every test added, renamed or removed named in the commit body. Nothing detects a deleted or widened test — `reps.size in 4..6` loosened to `3..7` is invisible to CI.
- **A test total is dated the moment it is written — name the SHA it was measured at, always.** State it as "`<N>`, measured at `<40-char SHA>`", never bare. This definition's own baseline is the proof: 71 was true when this file was first written at `e697787`, and the real count then moved 71 → 80 → 84 → 94 → 97 → 106 → 111 → 127 → 132 → 136 across the next dozen-odd commits (as recorded in each commit's own body, from `e665ea8` through `eb88bf3`) while a stale copy of 71 kept circulating in this file. The last several commits on `main` already name their parent SHA in the commit body for exactly this reason — require it the same way in a verdict: a bare number is a claim about a state that no longer exists by the time anyone reads it.
...
6. **What you verified yourself**, labelled for what it covers: "`ktlintCheck detekt test :app:lintDebug :app:assembleDebug` all green at `<SHA>`, 7 of 7 modules compiled, 136 tests / 0 failures (measured at that SHA); unverified: BLE link, sensor, device runtime, Room migration behaviour." If you took the faster `-PjvmOnly` path, say so and name the three modules you therefore did not compile — as a limitation of your choice, not a property of the machine. For CI, report every run for that SHA, and where there are two, phrase it honestly: "two runs of the same workflow on the same runner pool for one SHA: a flake check, not independent evidence."
