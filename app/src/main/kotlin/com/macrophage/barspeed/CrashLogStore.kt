...
class CrashLogStore(private val root: File) {
     ...
     * never be reached. It is here anyway because #273 was the same shape one
     * step along: `SetJournalStore.zipTo` read every stream whole, and on an
     * oversize journal the recovered capture was shared without its largest
     * stream. A share path that trusts a file to be small is exactly that
     * defect, and the trust is cheap to remove.
     *
