...
- "sensors" appears only on sets recorded with two accelerometers, and on sets where two units were connected and the app could not tell them apart. Read all five fields: "count" is how many streams the set armed, "expected" is the roles armed, "present" is the roles whose stream actually reached the raw zip, "analysedRole" says which one every figure in that set was computed from, and "shortfall" — "rolesUnassigned" or "rolesCollide" — says why two connected units produced one stream. A role absent from "present" captured nothing; if that role is the analysed one, the set's summary is empty rather than wrong. Absent "sensors" means one sensor, which is the normal case. There is no "plannedCount" any more and exports written before schema 1.15 carry one: it was what the plan asked for, and the plan no longer asks. The roles A and B are physical unit identities and carry no claim about which end of the bar or which hand — do NOT infer sides from them, and do not derive per-side metrics in this release: nothing is computed from the second stream yet, it is captured so that the analysis can be designed against real data.
    """.trimIndent()
