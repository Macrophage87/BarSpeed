...
- Exercise: {"exercise": snake_case_id, "sets": [...]} plus these optional keys:
  ...
  "sensorOnStack": true — the sensor is on the cable weight stack rather than on what I hold. The stack travels VERTICALLY no matter which way I move, so this is what decides which axis gets measured. DECLARE IT ANY TIME THE LOAD IS A PIN-SELECTED WEIGHT STACK AND THE SENSOR IS CLIPPED TO THAT STACK: assisted pull-up and dip machines, lat pulldown, seated row, leg press and hack squat sleds, leg curl and leg extension, and cable work in any plane all carry it; leave it false only for free weights and for a sensor worn on the lifter's body or clipped to the handle itself.
