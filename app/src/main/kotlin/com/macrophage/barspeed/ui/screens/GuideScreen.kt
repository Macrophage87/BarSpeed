...
- Exercise: {"exercise": snake_case_id, "sets": [...]} plus these optional keys:
  ...
  Those three geometry flags — "sensorInverted", "sensorOnStack" and "bodyweight" — read an OMITTED key differently from a declared false, and from schema 1.12 you may also write null, which means exactly what omitting it means. Omitting one is not the same as declaring it false: it says the plan has no opinion, and the app's own definition of that exercise stands — the stack mount it already knows for these exact ids: assisted_pull_up, assisted_chin_up, assisted_dip, lat_pulldown, seated_row, seated_cable_row, cable_row, triceps_pushdown, leg_curl, seated_leg_curl, lying_leg_curl, leg_extension, and nothing else, so on any other cable id an omitted key still resolves to false; body weight on pull_up, chin_up, dip, push_up, dead_hang, muscle_up, inverted_row and rope_dead_hang, and nothing at all behind "sensorInverted", which no built-in exercise carries, so leaving that one out still means false. Writing false IS an opinion and beats the app's own definition, which is what to write when the sensor was on the handle of a machine the app thinks is stack-mounted, or when the load on one of those eight ids is not my own body. The import gate names a line per exercise where it applied a built-in stack mount or a built-in body weight, so those two omissions are visible to me at import rather than only in the recorded numbers afterwards. There is no such line for "sensorInverted", because there is nothing built in for it to apply.
...
Two things the app does NOT accept, so don't invent them: there is no per-side shorthand (write one set object per side, with "side" — which also makes the real set count visible up front), and there is no way to mark prescribed work as skipped. A session that ends early simply has fewer sets in its export — but a set I DID record and did not perform stays in it carrying "voided": true, so read a missing set as never started and a voided one as recorded and not performed.
...
- Headroom end, on an exercise that progresses by WEIGHT (which is the default, and what an omitted "progression" means): 6 = could have added one equipment increment (10-15 lb / 5 kg), 4 = two increments (20-30 lb / 10 kg), 1 = could have added much more. These are LOAD, not reps: below three reps left a rep count is a guess, so the app stops asking for one.
- Headroom end, on an exercise that progresses by REPS: 6 = about 3-4 reps left, 4 = five or more left, 1 = many more. These START ABOVE the counted end, because 7-10 already cover three left and below, and one or two in reserve is my TARGET rather than headroom. The add-a-rep point is rung 4, not rung 6.
- Headroom end, on an exercise that progresses by TIME: 6 = could have gone about 15 s longer, 4 = about 30 s longer, 1 = much longer.
- Headroom end, on an exercise that progresses by NONE: 6 = comfortable, some left; 4 = easy, plenty left; 1 = very easy. A FEELING with no quantity, because that exercise holds its load and its reps and the app offers me nothing to add however I rate it.
- Timed sets (holds/carries): same seven rungs, with the counted end in that movement's words (7 = had more in me ... 10 = hit my limit). Their headroom end is asked in SECONDS unless the exercise declares "reps" or "none", which is the general rule above.
- Explosive sets: the counted end is bar speed (7 = fast and crisp, 8 = speed dropping, 10 = barely made it); the headroom end follows the exercise's "progression", as on any other lift.
- 2, 3 and 5 are valid values that have no tile. The gaps exist so the rungs sort; a value there is real, not corrupt.
- "rpeScale" = WHICH of those four scales a set was rated on: "load", "reps", "time" or "feel". Read a 1, 4 or 6 through it and never without it — the same integer is a plate claim on one exercise and a rep claim on another. It is frozen when the set is recorded, so it says what I was actually asked even if I have since edited or deleted the plan. It is ABSENT on every set exported before schema 1.19, where the rule is "load" on a dynamic set and "time" on a timed one — that is what the app asked then, and it says NOTHING about what that exercise progressed on, because nothing recorded it. It is ABSENT too on a set with no "rpe" at all.
- The TIMED wording moved at schema 1.19: a timed 6 used to mean "15-30 s longer" and a timed 4 "about a minute", and they now mean about 15 s and about 30 s. Nothing rewrote stored values, so do not compare timed headroom across that boundary.
...
- "voided": true = I recorded this set and DID NOT PERFORM it. Drop it from volume, from set counts and from every trend — its figures describe a row, not a performance. Everything the set always carried is still published: its load, its reps or hold, its prescription, its summary and its raw streams are exactly as they were recorded, and this key is what says not to read them as work that happened. "voidReason" carries my own words for why, when I had something to add, and sits beside the mark rather than inside it. The app NEVER derives this — it cannot tell a set that did not happen from one that failed instantly, and neither can you: only my own mark appears here. Both keys are ABSENT on a set I did not mark and on every set recorded before database v16, where the app could not ask — absent is "not marked", never "performed". A set that is simply MISSING from the session was never started; a voided one was recorded and not performed.
- "failed": true = failed set (missed reps, broke a hold early, or missed the lift) — treat as beyond RPE 10.
- "limiter" = why the set ended, in my own answer: muscle, grip, form, pace, slip, setup, pain, outside, other. Read "pain" apart from the rest — it is never a progression signal. Read "outside" as noise and DISCARD the set from capacity analysis; it means I was interrupted, not that I reached a limit. "grip" means train grip rather than dropping the main load; "form" is usually the opposite read from "muscle". "setup" means the set was set up wrong before it was ever a test of the muscle — a bad position, a seat or a pin at the wrong height — so its numbers are NOT a capacity reading: do not read the shortfall as a ceiling, and do not drop the load for it — I correct the set-up next session at the SAME load. "limiterNote" carries my own words and only ever sits beside "other". The key is ABSENT when I skipped the question, when the app never asked, and on everything exported before v0.1.45 — absent is never "ended for an unknown reason", so do not count it as a category. I am asked this on a set I FAILED and also on a set I finished but rated near failure (rpe 7-10), so a set carrying "limiter" did not necessarily fail — read "failed" for that. On a set I finished, the answer names what LIMITED the set rather than what ended it. I am not asked at all on a set I rated with room left (rpe 1, 4, 6), so its absence there says nothing.
...
- "rom_m" and everything derived from it (velocities, power) come from double integration and drift, badly on free weights where the sensor rotates. Compare them WITHIN a set and within a session; do not read them as calibrated distances or watts. "rollExcursion_deg" in the raw zip's meta.json tells you how far the sensor's roll swept: a few degrees means the kinematics are clean, and the more it swept the more gravity leaked into every sample. It is UNBOUNDED from v0.1.50 -- it is measured on an unwrapped signal, so a signal whose reported roll advanced through three turns reads about 1080, and a value near 360 is a real sweep rather than the old ceiling. It is not a claim about the barbell, the lifter or the mount: near |pitch| 90 degrees the sensor's own fused estimate can trade roll for yaw for the same physical attitude, so some of what this figure counts as swept angle can be that representation flipping rather than rotation. Older archives carry the OLD figure, which was max-minus-min over a bounded angle and could not exceed 360 whatever the mount did, so do not compare a pre-0.1.50 value against a later one. "rollExcursionBasis" beside it says which interval the figure covers: "workingWindow" (the lifter's work start to the terminal cue, which is what you want), "fromWorkStart" or "toTerminalCue" (only one bound was on the record), or "wholeCapture" (neither was, so the figure includes the seconds before I started and the seconds spent racking). Both keys are ABSENT together when the window held fewer than two samples -- that is "nothing measured whether it rotated", never zero rotation.
...
- "sensors" appears on sets recorded with two accelerometers, on sets where two units were PAIRED and the app could not tell them apart, AND on a single-sensor set whose one armed unit delivered too few frames to analyse across the whole set -- that third shape publishes "count" 1 with BOTH role lists EMPTY and carries "soleSilent". Read all eight fields: "count" is how many streams the set armed, "expected" is the roles armed, "present" is the roles whose stream actually reached the raw zip, "analysedRole" says which one every figure in that set was computed from, "analysedFellBack" is present and true only when that is NOT the role the set armed -- the armed unit delivered too few frames to analyse, another delivered enough, and the figures come from the one that did -- "shortfall" — "rolesUnassigned" or "rolesCollide" — says why two PAIRED units produced one stream, "silent" names each ARMED role that delivered too few frames to analyse across the whole set with one of "tooSoon", "notLinked", "linkWithoutSensor" or "linkedSilent", and "soleSilent" is that SAME vocabulary for a set whose one unroled link had no role to key a word by. "silent" and "soleSilent" are NEVER both written on one set -- they are one fact in two vocabularies, and which one a set gets depends on whether it armed a role at all. PAIRED IS NOT CONNECTED: the app decides this from the list of sensors it remembers, never from a live link, so a "shortfall" means two units are paired and cannot be told apart — NOT that both were switched on or in range; "silent" and "soleSilent" are each an OBSERVATION OF A LINK too, never a claim about a battery or a mounting. It also describes my DEVICE LIST rather than the set, so expect "shortfall" on every set of a session rather than on the ones that went wrong; one stale paired sensor writes it on every row. A role absent from "present" captured nothing, and a role PRESENT may still have delivered too few frames to analyse -- one frame is a file in the zip, and the app moves the analysis off a unit that sent fewer than eight only when another unit sent eight or more; "analysedFellBack" is the only thing that says it moved, and no field publishes a frame count. An analysed role absent from "present" never means "the surviving stream was analysed": it means no stream reached the analysis -- either nothing streamed at all, or every unit that streamed sent fewer than eight frames, so a capture may still be in the zip and the summary is empty either way -- or the set was recorded by a build that predates this key, whatever the document's "schemaVersion" says -- every document declaring 1.16 or earlier predates it -- when the analysis stayed on the armed unit whatever happened and published nothing derived when that unit was silent. Do not derive the fallback by comparing "analysedRole" with "present" -- wherever some unit sent eight frames or more the analysed role is in "present" either way, and where none did the analysis did not move at all, and "analysedFellBack" is the only thing that separates "analysed the unit I meant" from "analysed the only one that turned up". Treat a set that fell back as single-sensor data from the role it names. Absent "sensors" means one sensor that delivered a stream the analysis could run on, which is the normal case -- not merely one sensor, since a single armed unit that delivered too few frames to analyse now publishes this block too. There is no "plannedCount" any more and exports declaring 1.13 or 1.14 carry one inside this block: it was what the plan asked for, and the plan no longer asks. Exports declaring 1.12 or earlier have no "sensors" block at all, so its absence there is not a loss. And a set recorded before 1.15 that carried a "plannedCount" of 2 beside a "count" of 1 re-exports today as "count" 1 with an empty "expected", NO "shortfall" AND NO "soleSilent" — the reason it recorded one stream is gone from the document, so do not read that shape as "nothing was in the way"; a document that carries "soleSilent" instead is telling you what it saw, not omitting it. The roles A and B are physical unit identities and carry no claim about which end of the bar or which hand — do NOT infer sides from them, and do not derive per-side metrics in this release: exactly ONE stream is analysed per set and "analysedRole" says which, nothing is computed from the two together, and the stream that was not analysed is captured only so that the analysis can be designed against real data.
    """.trimIndent()
