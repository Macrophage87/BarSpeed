...
- Exercise: {"exercise": snake_case_id, "sets": [...]} plus these optional keys:
  ...
  "sensors": 1 | 2 — how many accelerometers each set of this exercise is recorded with. Default 1, and 1 is a permanent first-class choice, not a legacy one: a cable machine wants the single sensor on the stack. Declare 2 where two streams are wanted — one on each end of a barbell, or one in each hand on dumbbell work. A set can override it with its own "sensors", which wins. Two streams are only captured if I have two sensors paired AND have labelled each one A or B in the app against its own address; where I have not, the set records with one sensor and the export says two were asked for, so I can see the shortfall instead of guessing at it. A and B are the identity of the physical units and say NOTHING about which end of the bar or which hand each was on — I flip them constantly and that is fixed in post-processing, so never write a plan that depends on A being a particular side. I can change the count in the app and it comes back in the export as "count" beside "plannedCount".
...
- "sensors" appears only on sets recorded with two accelerometers, or on sets that ASKED for two and got one. Read all four fields: "plannedCount" is what the plan asked for, "count" is what was armed, "expected" is the roles armed, "present" is the roles whose stream actually reached the raw zip, and "analysedRole" says which one every figure in that set was computed from. A role absent from "present" captured nothing; if that role is the analysed one, the set's summary is empty rather than wrong. Absent "sensors" means one sensor, which is the normal case. The roles A and B are physical unit identities and carry no claim about which end of the bar or which hand — do NOT infer sides from them, and do not derive per-side metrics in this release: nothing is computed from the second stream yet, it is captured so that the analysis can be designed against real data.
    """.trimIndent()
