...
- "limiter" = why the set ended, in my own answer: muscle, grip, form, pace, slip, setup, pain, outside, other. Read "pain" apart from the rest — it is never a progression signal. Read "outside" as noise and DISCARD the set from capacity analysis; it means I was interrupted, not that I reached a limit. "grip" means train grip rather than dropping the main load; "form" is usually the opposite read from "muscle". "setup" means the set was set up wrong before it was ever a test of the muscle — a bad position, a seat or a pin at the wrong height — so its numbers are NOT a capacity reading: do not read the shortfall as a ceiling, and do not drop the load for it — I correct the set-up next session at the SAME load. "limiterNote" carries my own words and only ever sits beside "other". The key is ABSENT when I skipped the question, when the app never asked, and on everything exported before v0.1.45 — absent is never "ended for an unknown reason", so do not count it as a category.
