...
class AnchorSupplyByMountTest {
    ...
    fun `the contract - bar coverage clears the floor and no gate-holding capture moves`() {
        ...
        // 43, not the 28 that stood here: four landings enrolled captures
        // between them and the gate HOLDS on all but one of the sixteen.
        // Issue #301's three field-43 deadlift captures have gyro medians of
        // 5.250, 5.430 and 3.943 deg/s with tenth percentiles of 0.000, so
        // the MEDIAN sits under the 10 deg/s band and the capture does not
        // straddle it; 26-32 percent of samples are at or above the band and
        // the peaks reach 900-1280 deg/s. A loaded barbell hinge that does
        // not straddle, which #284's hazard H8 predicted it would. Issue
        // #259's three holds -- two rope dead hangs and a rope farmers hold
        // -- take the gate too, which is what a still or slowly carried
        // implement should do to it. Issues #290 and #255 then added
        // field-42's six sets, on five of which the gate holds;
        // `GyroGateTest` carries their medians and tenth percentiles. The
        // sixth, field-42 set 2, straddles (median 14.68 deg/s, tenth
